...
FONT_PATH = "Days.ttf"  # Используем шрифт Days. Убедитесь, что файл Days.ttf есть в папке!
CARD_SIZE = (1655, 2340)
...
def generate_cards(excel_file_path, output_pdf_path):
    ...
    try:
        ...
        try:
            ...
            print(f"Ошибка: Не найден шрифт по пути '{FONT_PATH}'. Пожалуйста, убедитесь, что файл существует.")
            return None
        ...
        for index, row in data.iterrows():
            ...
            for field in ['Имя', 'Фамилия']:
                ...
                align = FIELD_SETTINGS[field].get('align')
...
def handle_document(message):
    try:
        ...
        if pdf_file_path:
            ...
            try:
                os.remove(pdf_file_path) # Попытка удалить временный PDF
            except Exception as e:
                print(f"Ошибка при удалении временного PDF: {e}")
        else:
